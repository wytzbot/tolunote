...
//   1. The browser's "Ask AI" modal — a paying user's request. Gated
//      server-side on an active $2/month subscription so the paywall
...
//      secret rather than a subscription.
//
...
async function requireActiveSubscriptionOrInternalCall(req) {
    ...
    try {
        ...
        const active = !!(sub && sub.active && (!sub.currentPeriodEnd || sub.currentPeriodEnd > Date.now()));
        if (!active) {
            return { ok: false, status: 402, error: 'The AI assistant requires an active $2/month subscription.' };
        }
        return { ok: true };
    } catch (err) {
...
export default async function handler(req, res) {
    ...
    const gate = await requireActiveSubscriptionOrInternalCall(req);
    if (!gate.ok) {

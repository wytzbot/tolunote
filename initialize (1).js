...
//
// Starts a Paystack subscription checkout for the ₦3,000/month premium plan
...
//   PAYSTACK_CURRENCY             optional, defaults to "NGN". Must be a
//                                  currency enabled on your Paystack account.

import { getAdminDb, getAdminAuth } from '../_lib/firebaseAdmin.js';

const PLAN_AMOUNT = 300000; // ₦3,000.00 in the smallest currency unit (kobo)
...
// the ₦3,000/month plan once and caches its code in Firestore.
async function getOrCreatePlanCode(db) {
    ...
    if (snap.exists && snap.data().planCode && snap.data().currency === currency) {

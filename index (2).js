...
// Lets this system call through /api/chat's ad-unlock gate without a
...
exports.deliverDueReminders = onSchedule('every 1 minutes', async () => {
    ...
    for (const reminderDoc of dueSnap.docs) {
        ...
        try {
            // Access to the *reminders* feature is gated at creation time (an
            // active ad-unlock was required to create the doc — see
            // firestore.rules.txt). Delivery itself isn't re-gated here: once
            // a reminder exists, it should fire on schedule even if the
            // short-lived ad-unlock window has since expired — otherwise a
            // reminder set today for next week would silently never arrive.
